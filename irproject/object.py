...
""" 你的 APPID AK SK """
APP_ID = '33024237'
API_KEY = 'XKXOVTm9hrpevY5FModZGVp0'
SECRET_KEY = 'G7IoqShz2Q9C9gtfHo9YwT1vVX2x0Ofa'
...
# 添加标题标签
title_label = tk.Label(window, text="欢迎进入通用识别系统", font=("宋体", 32, "bold"), bg="#00BFFF", fg="#FFFFFF")
...
def recognize_and_preview():
    ...
    for item in result['result']:
        index = i
        result_treeview.insert('', 'end', values=(i, item['keyword'], str(round(item['score'] * 100, 2)) + '%'))
        result_treeview.bind(f'<Double-Button-1>',
                             lambda event, index=index, keyword=item['keyword']: open_browser_for_search(keyword))
        i += 1
